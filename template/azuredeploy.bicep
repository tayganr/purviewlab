...
//  az deployment group create --name ExampleDeployment --resource-group pvdemo-rg-21lck --template-file azuredeploy.json

// Azure VM
resource vm 'Microsoft.Compute/virtualMachines@2021-04-01' = {
  name: 'pvlab-${randomString}-vm'
  location: location
  properties: {
...
resource sqlvm 'Microsoft.SqlVirtualMachine/sqlVirtualMachines@2017-03-01-preview' = {
  name: 'pvlab-${randomString}-sqlvm'
  location: location
  properties: {
    virtualMachineResourceId: resourceId('Microsoft.Compute/virtualMachines', 'pvlab-${randomString}-sqlvm')
